...
const { pacalCase, pascalCase } = require('pascal-case')
...
function convertToToc(source, data) {
    ...
    for (var httpPath in source.paths) {
        for (var httpMethod in source.paths[httpPath]) {
            if ((httpMethod !== 'parameters') && (httpMethod !== 'summary') && (httpMethod !== 'description')) {
                var method = {
                    operation: source.paths[httpPath][httpMethod],
                    pathItem: source.paths[httpPath],
                    verb: httpMethod,
                    path: httpPath,
                    pathParameters: source.paths[httpPath].parameters
                }

                if (!method.operation['x-public']) {
                    continue
                }

                var sMethodUniqueName = (method.operation.operationId ? method.operation.operationId : httpMethod + '_' + httpPath).split('?')[0].split('/').join('_');
        ...
        if (!r.startsWith('x-')) {
            ...
            for (let ct in response.content) {
                ...
                if (contentType.schema && contentType.schema["x-widdershins-oldRef"] && contentType.schema["x-widdershins-oldRef"].startsWith('#/components/')) {
                    ...
                    entry.schema = '[' + schemaName + '](#' + schemaName.toLowerCase() + ')';
                    entry.$ref = true;
                }
                else {
...
function getResponseExamples(data) {
    ...
    for (let example of examples) {
        if (example.description && example.description !== lastDesc) {
            content += '::: tip ' + example.description + '\n:::\n\n';
...
function convertEndpoints(api, options) {
    return new Promise(function (resolve, reject) {
        ...
        ({ host: data.host, protocol: data.protocol } = new URL(data.servers[0].url));
        ...
        Object.entries(data.resources).forEach(([_, resource]) => {
            Object.entries(resource.methods).forEach(([key, value]) => {
                let content = '';
                ...
                data.operationUniqueName = key;
                data.method = value;
                data.operationUniqueSlug = data.method.slug;
                data.operation = data.method.operation;

                content += templates.operation(data);
                content = common.removeDupeBlankLines(content);

                data.operation.tags.forEach(tag => {
                    if (!endpoints[tag]) {
                        endpoints[tag] = {}
                    }

                    endpoints[tag][key] = content
                })
            })
        })
